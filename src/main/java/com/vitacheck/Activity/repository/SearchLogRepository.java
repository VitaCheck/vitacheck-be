...
public interface SearchLogRepository extends JpaRepository<SearchLog, Long>, SearchLogRepositoryCustom {
    ...
    @Query("SELECT s.keyword FROM SearchLog s " +
            ...
            "AND s.method =com.vitacheck.Activity.domain.SearchLog.Method.SEARCH " +
            "GROUP BY s.keyword " +
    ...
    @Query("SELECT s.keyword FROM SearchLog s " +
            "WHERE s.userId = :userId " +
            "AND s.method =com.vitacheck.Activity.domain.SearchLog.Method.CLICK " +
            "AND s.category =com.vitacheck.Activity.domain.SearchLog.SearchCategory.SUPPLEMENT " +
            "GROUP BY s.keyword " +
            "ORDER BY MAX(s.createdAt) DESC")
